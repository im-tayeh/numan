...
(function () {
  ...
  // يسابق الطلب مع مؤقّت: إن تعلّق الطلب (لا فشل ولا نجاح) نرفض بعد المهلة فيسقط للوضع الأوفلاين
  function withTimeout(p, ms){ return Promise.race([ Promise.resolve(p), new Promise(function(_,rej){ setTimeout(function(){ rej(new Error('timeout')); }, ms||8000); }) ]); }
  ...
  let flushingScan=false, flushingOps=false, flushErrShown=false;
  ...
  async function flushOps(sb){
    ...
    flushingOps=true; const remain=[]; let lastErr=null;
    for(const op of a){
      try{
        let error=null;
        ...
        if(error){ remain.push(op); lastErr=error; }   // رفض من الخادم
      }catch(e){ remain.push(op); }                      // انقطاع فعلي — لا يُعتبر خطأ خادم
    }
    ...
    // عند الاتصال: إن بقيت عناصر فشلت برفض الخادم، أظهر السبب الحقيقي (مرة واحدة) بدل التعليق الصامت
    if(lastErr && navigator.onLine && !flushErrShown && typeof window.__numanToast==='function'){ flushErrShown=true; window.__numanToast('تعذّر رفع '+remain.length+' عملية على الخادم: '+(lastErr.message||lastErr.code||'')+' — شغّل numan_E_fixes.sql',1); }
  }
  ...
  async function identity(sb, uid, key){
    if(navigator.onLine){
      try{ const r=await withTimeout(sb.from('staff').select('*').eq('auth_uid',uid).maybeSingle()); if(r.data){ cacheSet('me_'+key, r.data); return r.data; } }catch(e){}
    }
  ...
  async function load(key, fetchFn){
    if(navigator.onLine){
      try{ const data=await withTimeout(fetchFn()); cacheSet('d_'+key, data); return { data, offline:false }; }catch(e){}
    }
  ...
  async function insertOrQueue(sb, table, payload){
    if(navigator.onLine){
      try{ const {data,error}=await withTimeout(sb.from(table).insert(payload).select()); if(!error) return { online:true, data }; }catch(e){}
    }
    addOp({ kind:'insert', table, payload }); return { online:false };
  }
  async function delOrQueue(sb, table, col, val){
    if(navigator.onLine){
      try{ const {error}=await withTimeout(sb.from(table).delete().eq(col,val)); if(!error) return { online:true }; }catch(e){}
    }
  ...
  async function committeeBoot(sb, session, committeeName, key){
    ...
    if(navigator.onLine){
      try{
        const c=await withTimeout(sb.from('committees').select('id').eq('name',committeeName).maybeSingle()); com=c.data; cacheSet('com_'+key, com||null);
        if(com){ const m=await withTimeout(sb.from('committee_members').select('role').eq('committee_id',com.id).eq('staff_code',me.code).maybeSingle()); mem=m.data||null; cacheSet('mem_'+key, mem); }
  ...
  window.NumanQueue={
    ...
    cacheGet, cacheSet, banner, identity, load, insertOrQueue, delOrQueue, committeeBoot, withTimeout,
    async record(sb, code, date, checkin){
      if(navigator.onLine){ try{ const {data,error}=await withTimeout(sb.rpc('scan_attendance',{p_code:code,p_date:date,p_checkin:checkin})); if(!error) return { online:true, data }; }catch(e){} }
